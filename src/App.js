...
class App extends Component {
  state = {
    score: 0,
    timeLeft: 60, // change to 60
    imageToBeMatched: imagesList[0].imageUrl,
    imageToBeMatchedId: imagesList[0].id,
  ...
  filterActiveCategoryThumbnails = () => {
    const {activeTabId} = this.state
    return imagesList.filter(each => each.category === activeTabId)
  }

  selectedThumbnail = id => {
    const {imageToBeMatchedId} = this.state

    if (imageToBeMatchedId === id) {
      const newImageOnBoard =
        imagesList[Math.floor(Math.random() * imagesList.length)]

      this.setState({
        imageToBeMatchedId: newImageOnBoard.id,
        imageToBeMatched: newImageOnBoard.imageUrl,
      })
  ...
  render() {
    const {
      ...
      imageToBeMatched,
      activeTabId,
      carriageOfImageUrlsGeneratedIds,
    } = this.state

    console.log(carriageOfImageUrlsGeneratedIds)
          ...
          <ul className="nav-items-container">
            ...
            <li className="nav-item">
              <img
                ...
                alt="timer"
              />
              <p className="time-count-down">{timeLeft} sec</p>
            </li>
          </ul>
        </nav>
          ...
          {timer ? (
            <>
              <div className="big-image-container">
                <img className="big-image" src={imageToBeMatched} alt="match" />
              </div>
              <ul className="tabs-container">
                {tabsList.map(each => {
                  const selectClass =
                    each.tabId === activeTabId ? 'selected-tab' : ''
                  const selectingCategory = () => {
                    this.changeCategory(each.tabId)
                  }
                  return (
                    <li key={each.tabId}>
                      <button
                        className={`tab-button ${selectClass}`}
                        type="button"
              ...
              <ul className="thumbnails-container">
                {this.filterActiveCategoryThumbnails().map(each => {
                  const thumbnailClicked = () => {
                    this.selectedThumbnail(each.id)
                  }
                  return (
                    <li className="thumbnail-container" key={each.id}>
                      <button
